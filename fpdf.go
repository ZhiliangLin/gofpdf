...
func (f *Fpdf) replaceAliases() {
	for mode := 0; mode < 2; mode++ {
		for alias, replacement := range f.aliasMap {
			...
			for n := 1; n <= f.page; n++ {
				s := f.pages[n].String()
				totalPageWidth := f.GetStringWidth(strings.Replace(replacement, "\x00", "", -1))
				aliasWith := f.GetStringWidth(strings.Replace(alias, "\x00", "", -1))
				...
				if match != "" {
					...
					locNum = locNum + f.UnitToPointConvert(aliasWith) - f.UnitToPointConvert(totalPageWidth)
